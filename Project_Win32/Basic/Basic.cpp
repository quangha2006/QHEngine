...
void Basic::Update(int delta)
{
	...
	for (int i = 0; i < numModelcircle; i++)
	{
		...
		if (currPos.y < 0.f)
		{
			uvcircles[i].SetPos(glm::vec3(0.f, 5.0f, 0.0f));
...
bool Basic::OnGameKeyPressed(int key, int scancode, int action, int mods)
{
	...
	case 328:
		...
		mBoblampclean.SetPhycicsMode(PhycicsMode_Kinematic);
		return true;
	case 329: // num 9
		//mSpider.SetAnimPlay(9);
		mBoblampclean.SetPhycicsMode(PhycicsMode_Dynamic);
		return true;
