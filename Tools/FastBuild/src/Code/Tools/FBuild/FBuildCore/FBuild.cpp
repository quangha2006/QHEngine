    ...
    , m_ImportedEnvironmentVars( 0, true )
{
	...
	m_DependencyManager = FNEW(DependencyManager);
}
...
bool FBuild::Initialize( const char * nodeGraphDBFile )
{
	...
	// this needs to be after setting the single threaded mode for SBA
	...
	for (DependencyBFFInformation info : m_DependencyManager->GetDependiciesFileInfo())
	{
		AString db(info.m_BFFFilePath);
		db.Replace(".bff", ".fdb");
		m_DependencyGraph->AddUsedDBFile(db);
		m_DependencyGraph->AddUsedFile(info.m_BFFFilePath, info.m_TimeStamp, info.m_DataHash);
    }
...
bool FBuild::Build( const Array< AString > & targets )
{
	...
	{
		Node* aliasNode = m_DependencyGraph->FindNode(dep.GetNode()->GetName());
		Dependency* mainDependency = aliasNode->GetStaticDependencies().Begin();
		if (mainDependency)
		{
			Node* targetNode = mainDependency->GetNode();
			for (const AString& depName : targetNode->GetPrebuildDependenciesName())
			{
				ResultedDependency resDep = m_DependencyManager->GetNodeFromBuildDep(depName.Get());
				if (resDep.IsValid())
				{
#if defined( __WINDOWS__ )
					...
					AStackString<> output;
					output += "Dependency: ";
					output += resDep.m_Node->GetName();
					output += '\n';
					FLOG_BUILD_DIRECT(output.Get());
					SetConsoleTextAttribute(hConsole, 15);
#endif
					bool buildres = FBuild::Get().Build(resDep.m_Node, resDep.m_NodeGraph);
					if (!buildres)
						return false;
				}
			}
...
void FBuild::MergeNodeGraphsRecurse(NodeGraph* merge, Dependencies & dependencies, Array< bool > & buildAdded) const
{
	for (Dependency dep : dependencies)
	{
		if (!buildAdded[dep.GetNode()->GetIndex()])
		{
			
			FLOG_WARN("Add nod cu index %i si nume %s", dep.GetNode()->GetIndex(), dep.GetNode()->GetName().Get())
			if (dep.GetNode()->GetIndex() == 496)
			{
				int a = 0;
				a++;
			}
			
			MergeNodeGraphsRecurse(merge, dep.GetNode(), buildAdded);
			merge->AddNode(dep.GetNode(), false);
			buildAdded[dep.GetNode()->GetIndex()] = true;
		}
	}
}

void FBuild::MergeNodeGraphsRecurse(NodeGraph* merge, Node* build, Array< bool > & buildAdded) const
{
	if (!buildAdded[build->GetIndex()])
	{
		FLOG_WARN("Add nod cu index %i si nume %s", build->GetIndex(), build->GetName().Get())

		Dependencies prebBuildDeps = build->GetPreBuildDependencies();
		MergeNodeGraphsRecurse(merge, prebBuildDeps, buildAdded);

		Dependencies staticDeps = build->GetStaticDependencies();
		MergeNodeGraphsRecurse(merge, staticDeps, buildAdded);

		Dependencies dynamicDeps = build->GetDynamicDependencies();
		MergeNodeGraphsRecurse(merge, dynamicDeps, buildAdded);

		buildAdded[build->GetIndex()] = true;
	}
}

NodeGraph* FBuild::MergeNodeGraphs(NodeGraph* disk, NodeGraph* built) const
{
	NodeGraph *mergeNG = FNEW(NodeGraph);

	const size_t numNodes = built->m_AllNodes.GetSize();
	Array< bool > visited(numNodes, false);
	visited.SetSize(numNodes);
	memset(visited.Begin(), 0, numNodes);

	for (Node* diskNode : disk->m_AllNodes)
	{
		//if we found a node already it means it was already added either by the Build NodeGraph or what's saved on disk
		if (mergeNG->FindNode(diskNode->GetName()))
		{
			continue;
		}

		Node* buildNode = built->FindNode(diskNode->GetName());
		if (!buildNode)
		{
			FLOG_WARN("Add nod ce nu e in BUILT cu index %i si nume %s", diskNode->GetIndex(), diskNode->GetName().Get())
			mergeNG->AddNode(diskNode,false);
		}
		else
		{
			mergeNG->AddNode(buildNode, false);
			MergeNodeGraphsRecurse(mergeNG, buildNode,visited);
		}
	}
...
// SaveDependencyGraph
//------------------------------------------------------------------------------
bool FBuild::SaveDependencyGraph(NodeGraph* nodeGraph, const char * nodeGraphDBFile)
{
	...
	nodeGraph->Save(memoryStream, nodeGraphDBFile);
	...
	AStackString<> output;
	output += "Saving Dependency: ";
	output += nodeGraphDBFile;
	output += '\n';
	FLOG_BUILD_DIRECT(output.Get());
	SetConsoleTextAttribute(hConsole, 15);
#endif
...
bool FBuild::Build(Node* nodeToBuild, NodeGraph* dependcyGraph)
{
	ASSERT(nodeToBuild);
	...
	for (;;)
	{
		...
		m_JobQueue->FinalizeCompletedJobs(*dependcyGraph);

		if (!stopping)
		{
			// do a sweep of the graph to create more jobs
			dependcyGraph->DoBuildPass(nodeToBuild);
		}
	...
	{
		SaveDependencyGraph(dependcyGraph, dependcyGraph->GetFDBPathFile().Get());
	}
...
//W: GetDataPath : modify to relative path when Python is the compiler
//------------------------------------------------------------------------------
