...
// Build
// Funtions for Color output
//------------------------------------------------------------------------------
/*static*/ void FLog::Build_Color( WORD color, const char * formatString, ... )
{
	AStackString< 8192 > buffer;
...
/*static*/ void FLog::Error( const char * formatString, ... )
{
	...
	buffer.VFormat( formatString, args );
	va_end( args );
	// Set for Red output on errors
	ColorConsoleScope Red(RED_C);
