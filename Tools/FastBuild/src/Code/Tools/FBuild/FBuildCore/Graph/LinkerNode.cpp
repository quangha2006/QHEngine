...
REFLECT_NODE_BEGIN( LinkerNode, Node, MetaName( "LinkerOutput" ) + MetaFile() )
	...
	//W: Use Optional for "Libraries" paramaters
	REFLECT_ARRAY( m_Libraries,					"Libraries",					MetaOptional() + MetaFile() + MetaAllowNonFile())
...
/*virtual*/ bool LinkerNode::Initialize( NodeGraph & nodeGraph, const BFFIterator & iter, const Function * function )
{
    ...
    // .Linker
	m_BasePreBuildDependencyNames = m_PreBuildDependencyNames;
...
bool LinkerNode::CanUseResponseFile() const
{
        ...
        return ( GetFlag( LINK_FLAG_MSVC ) || GetFlag( LINK_FLAG_GCC ) || GetFlag( LINK_FLAG_SNC ) || GetFlag( LINK_FLAG_ORBIS_LD ) || GetFlag( LINK_FLAG_GREENHILLS_ELXR ) || GetFlag( LINK_FLAG_CODEWARRIOR_LD ) || GetFlag(LINK_FLAG_CLANG) );
    #else
        return false;
    #endif
}
...
void LinkerNode::Sync(const LinkerNode* node)
{
	m_LinkerStampExeArgs = node->m_LinkerStampExeArgs;

	m_LinkerType = node->m_LinkerType;
	m_Linker = node->m_Linker;
	m_LinkerOptions = node->m_LinkerOptions;
	m_Flags = node->m_Flags;

	m_ImportLibName = node->m_ImportLibName;
	m_LinkerStampExeArgs = node->m_LinkerStampExeArgs;
}
//------------------------------------------------------------------------------
