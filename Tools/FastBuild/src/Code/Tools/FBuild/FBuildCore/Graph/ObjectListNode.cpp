...
bool ObjectListNode::CreateDynamicObjectNode( NodeGraph & nodeGraph, Node * inputFile, const AString & baseDir, bool isUnityNode, bool isIsolatedFromUnityNode )
{
    ...
    if (objNode && objNode->GetSourceFile()->GetName().Compare(inputFile->GetName()) != 0)
    {
        const char * lastSlash = inputFile->GetName().FindLast( NATIVE_SLASH );
        lastSlash = lastSlash ? ( lastSlash + 1 ) : inputFile->GetName().Get();

        const char * lastDot = m_CompilerOutputKeepBaseExtension ? inputFile->GetName().GetEnd() : inputFile->GetName().FindLast( '.' );
        lastDot = lastDot && ( lastDot > lastSlash ) ? lastDot : inputFile->GetName().GetEnd();

        const char * extension = inputFile->GetName().FindLast( '.' );
        ...
        FLOG_BUILD("%s for source file %s\n",objFile.Get(),inputFile->GetName().Get());
