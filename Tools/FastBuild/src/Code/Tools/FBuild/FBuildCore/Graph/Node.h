...
protected:
	...
	inline const Array< AString >& GetPrebuildDependenciesName() { return m_BasePreBuildDependencyNames;  }

	virtual void RemoveDependency(const Dependency& n, bool isStatic = false)
	{
		if (isStatic)
		{
			m_StaticDependencies.FindAndErase(n);
		}
		else
		{
			m_DynamicDependencies.FindAndErase(n);
		}
    ...
    uint32_t        m_Index;
    bool            m_Hidden;
	Array< AString > m_BasePreBuildDependencyNames;
