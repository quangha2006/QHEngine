...
#include "Tools/FBuild/FBuildCore/Managers/DependencyManager.h"
                                               ...
                                               bool forceMigration )
{
	...
	oldNG->SetFDBPathFile(nodeGraphDBFile);
    // What happened?
    switch ( res )
    {
        case LoadResult::MISSING_OR_INCOMPATIBLE:
        {
			...
			newNG->SetFDBPathFile(nodeGraphDBFile);
            return newNG;
        }
        ...
        case LoadResult::OK:
        {
			//but we still need to populate the dependency manager to know what we are dependent of
			//if the FDB is found than we added to the dep, otherwise we e
            // Nothing more to do
			FBuild::Get().GetDependencyManager()->InitializeMissingDepencies(bffFile);
            return oldNG;
        }
    }
...
NodeGraph::LoadResult NodeGraph::Load( IOStream & stream, const char * nodeGraphDBFile )
{
	...
	bool partialBFFChange = false;
    Array< UsedFile > usedFiles;
	Array< AString>	usedDatabases;
	if (ReadHeaderAndUsedFiles(stream, nodeGraphDBFile, usedFiles, usedDatabases, compatibleDB) == false)
    {
        return LoadResult::LOAD_ERROR;
    }
    ...
    for ( size_t i=0; i<usedFiles.GetSize(); ++i )
    {
        ...
        if ( !bffNeedsReparsing )
        {
            FLOG_WARN( "MAIN BFF file '%s' has changed (reparsing will occur).", fileName.Get() );
            bffNeedsReparsing = true;
        }
    }

    m_UsedFiles = usedFiles;
	m_UsedDatabases = usedDatabases;
...
void NodeGraph::Save( IOStream & stream, const char* nodeGraphDBFile ) const
{
	...
	// write used file
	uint32_t numDBFile = (uint32_t)m_UsedDatabases.GetSize();
	stream.Write(numDBFile);
	for (uint32_t i = 0; i < numDBFile; ++i)
	{
		const AString & fileName = m_UsedDatabases[i];
		uint32_t fileNameLen(fileName.GetLength());
		stream.Write(fileNameLen);
		stream.Write(fileName.Get(), fileNameLen);
	}
...
/*static*/ void NodeGraph::SerializeToText( Node * node, uint32_t depth, AString& outBuffer )
{
    ...
    outBuffer.AppendFormat( "%*s%s %s %i\n", depth * 4, "", node->GetTypeName(), node->GetName().Get(),node->GetIndex());
...
// AddNode
//------------------------------------------------------------------------------
void NodeGraph::AddNode(Node * node, bool setIndex)
{
    ASSERT( Thread::IsMainThread() );
...
void NodeGraph::AddUsedDBFile(const AString& dbfile)
{
	m_UsedDatabases.Append(dbfile);
}
...
//------------------------------------------------------------------------------
bool NodeGraph::ReadHeaderAndUsedFiles( IOStream & nodeGraphStream, const char* nodeGraphDBFile, Array< UsedFile > & files, Array<AString> & dbFiles, bool & compatibleDB ) const
{
	...
	uint32_t dbNrFiles = 0;
	if (!nodeGraphStream.Read(dbNrFiles))
	{
		return false;
	}
	for (uint32_t i = 0; i < dbNrFiles; ++i)
	{
		uint32_t dbFileLen(0);
		if (!nodeGraphStream.Read(dbFileLen))
		{
			return false;
		}
		AStackString<> dbfileName;
		dbfileName.SetLength(dbFileLen); // handles null terminate
		if (nodeGraphStream.Read(dbfileName.Get(), dbFileLen) != dbFileLen)
		{
			return false;
		}

		dbFiles.Append(dbfileName);
	}
