...
public:
	...
	void AddUsedDBFile(const AString& db);
	...
	void SetFDBPathFile(const char* path)
	{
		m_FDBPath = path;
	}

	const AString& GetFDBPathFile() { return m_FDBPath; }
	const Array< AString >& GetUsedFDB() { return m_UsedDatabases; }
private:
    friend class FBuild;
    ...
    void AddNode( Node * node, bool setIndex = true );
    ...
    struct UsedFile;
	bool ReadHeaderAndUsedFiles(IOStream & nodeGraphStream, const char* nodeGraphDBFile, Array< UsedFile > & files, Array<AString> & dbFiles, bool & compatibleDB)  const;
    ...
    struct UsedFile
    {
		explicit UsedFile(const AString & fileName, uint64_t timeStamp, uint64_t dataHash) : m_FileName(fileName),
