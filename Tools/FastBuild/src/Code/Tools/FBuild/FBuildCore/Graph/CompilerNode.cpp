...
REFLECT_NODE_BEGIN( CompilerNode, Node, MetaNone() )
	...
	//W: Use Simple Cache mode for Special compilations
	REFLECT( m_SimpleCacheMode,  "SimpleCacheMode",   MetaOptional() )
...
bool CompilerNode::InitializeCompilerFamily( const BFFIterator & iter, const Function * function )
{
    ...
    if ( m_CompilerFamilyString.EqualsI( "auto" ) )
    {
	    ...
	    {
		    m_CompilerFamilyEnum = PYTHON; // thien.nc add for python build script
            return true;
	    }
