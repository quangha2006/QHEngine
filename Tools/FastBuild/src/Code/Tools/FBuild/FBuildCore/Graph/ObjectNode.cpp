...
REFLECT_NODE_BEGIN( ObjectNode, Node, MetaNone() )
    ...
    REFLECT_ARRAY( m_PreBuildDependencyNames,       "PreBuildDependencies",             MetaOptional() + MetaAllowNonFile() )
...
Node::BuildResult ObjectNode::DoBuildWithPreProcessor( Job * job, bool useDeoptimization, bool useCache, bool useSimpleDist )
{
	...
	/*const bool belowMemoryLimit = true; ( ( Job::GetTotalLocalDataMemoryUsage() / MEGABYTE ) < FBuild::Get().GetSettings()->GetDistributableJobMemoryLimitMiB() );*/
    if ( canDistribute  )
    {
        // compress job data
        Compressor c;
                                                ...
                                                bool usingPCH )
{
    ...
    switch ( compilerFamily )
    {
		...
		//W: Handle Python as External Compiler
    ...
    if ( flags & ( ObjectNode::FLAG_CLANG | ObjectNode::FLAG_GCC | ObjectNode::FLAG_SNC | ObjectNode::CODEWARRIOR_WII | ObjectNode::GREENHILLS_WIIU ) )
    {
        ...
        //ms it seems that something fails with clang and pch when using cacheing so let's not cache them for now 
        // Ideally we should check the actual problem ... maybe when we get the pch from the cache we need to update it's timestamp ??
        bool isClang = flags & ( ObjectNode::FLAG_CLANG );
        bool isCreatingPCH =  flags & (ObjectNode::FLAG_CREATING_PCH);
	...
	//W: thien.nc add to allow caching when using .SimpleCacheMode
...
/*static*/ void ObjectNode::HandleSystemFailures( Job * job, int result, const char * stdOut, const char * stdErr )
{
    ...
    if ( result == 0 )
    {
		...
		//W: If remote PC doesn't have tool installed, compiler can be terminated
